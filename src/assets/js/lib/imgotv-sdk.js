(function (_w) {
    ...
    function createCallbackFunction(funcName, callbackFunc) {
        ...
        /**********************************/
        /* 兼容客户端的登录回调异常逻辑，需要保证对应接口每次被成功调用完毕 */
        if (funcName == 'login_successCallback' ||
            funcName == 'getUserInfo_successCallback') {
            funcName = 'getUserInfo_successCallback0';
            window[funcName] = callbackFunc;
            return funcName;
        }
        /**********************************/

        if (typeof window[funcName + 1] != 'function') {
            window[funcName + 1] = callbackFunc;
            __functionIndexMap[funcName] = 1;
            return funcName + 1
...
(function (_w) {
    ...
    var
        IA = {
            ...
            changeVideo: function (opt) {
                ...
                Invoker.use('changeVideo', {
                    param: {
                        ...
                        category: opt.category || 'summershow2015',
                        type: opt.type || 1,
                        hUrl: opt.hUrl || ''
                    }
                })
            ...
            jumpOtherApp: function (opt) {
                if (typeof(opt) !== 'object') return;

                Invoker.use('jumpotherapp', {
                    param: {
                        jumpurl: opt.jumpurl || '',
